...
if ~isempty(im_array{1,1}) && ~isempty(im_array_tune{1,1}) && cur_trial > 0
for ij = 1:num_planes
    ...
    if max(max(im_array_tune_var{plot_planes(ij),chan_num}(:,:,cur_trial)))~=0
        r = r.*(1-im_array_tune_var{plot_planes(ij),chan_num}(:,:,cur_trial)/max(max(im_array_tune_var{plot_planes(ij),chan_num}))).^(c_lim_overlay/256);
    end
