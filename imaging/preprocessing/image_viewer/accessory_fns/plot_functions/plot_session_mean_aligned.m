function [im_comb clim] = plot_session_mean_aligned(plot_axes,cbar_axes,im_session,ref,trial_num,chan_num,plot_planes,clim,c_lim_overlay,plot_on)
...
for ij = 1:num_planes
	...
	if trial_num > 0
		im_use = squeeze(mean(im_session.reg.align_mean(:,:,plot_planes(ij),chan_num,1:trial_num),5));
	else
		im_use = ref.session_mean{plot_planes(ij),chan_num};
	end
