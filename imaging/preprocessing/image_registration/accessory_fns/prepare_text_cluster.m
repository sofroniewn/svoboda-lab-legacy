...
if isempty(start_ind)
	...
	if ~isempty(im_session.reg.nFrames)
		...
		evalScript = sprintf('qsub -t %g-%g -pe batch 16 -N ''textConvert'' -j y -o /dev/null -b y -cwd -V ''%s %s ${SGE_TASK_ID} > %s${SGE_TASK_ID}.log %s %s %s %s %s''',startTime, endTime, functionPath, directory, logPath, analyze_chan, down_sample, num_files, num_frames, num_pixels);
		display(evalScript)
	else
