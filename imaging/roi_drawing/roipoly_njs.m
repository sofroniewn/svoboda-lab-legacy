...
%   ROIPOLYOLD is an old version of ROIPOLY.  It uses an algorithm
...
%   $Revision.4 $  $Date: 2011/07/19 23:54:38 $

%warning(message('images:removing:function','ROIPOLYOLD','ROIPOLY'))

[xdata,ydata,num_rows,num_cols,xi,yi] = parse_inputs(varargin{:});

if length(xi)~=length(yi)
    error(message('images:roipolyold:xiyiMustBeSameLength')); 
end
...
otherwise
    error(message('images:roipolyold:tooManyOutputArgs'));
...
case 0, 
    ...
    if ~hasimage,
        error(message('images:roipolyold:needImageInFigure'));
    end
    [xi,yi] = getline_njs(gcf,'closed'); % Get rect info from the user.
    nrows = size(a,1);
    ncols = size(a,2);
...
otherwise,
    error(message('images:roipolyold:invalidInputArgs'));
