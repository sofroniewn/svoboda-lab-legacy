function failed = func_spike_sort_klusters(batch_mode, over_write, over_write_spikes, over_write_cluster)
%%

disp(['--------------------------------------------']);
failed = [];
for i_batch = 1:numel(batch_mode)
    disp(['START BATCH ' num2str(i_batch)]);
    
    try
        all_base_dir = batch_mode{i_batch}.base_dir;
        file_nums = batch_mode{i_batch}.file_nums;
        cluster_name = batch_mode{i_batch}.cluster_name;
