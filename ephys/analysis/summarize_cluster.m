...
for ij = 1:length(all_clust_ids)
	...
	if plot_on
        subtightplot(2,8,[1 9],gap,marg_h,marg_w)
    ...
    if plot_on
        subtightplot(2,8,[2 10],gap,marg_h,marg_w)
    ...
    if plot_on
        subtightplot(2,8,[3 4],gap,marg_h,marg_w)
    ...
    if plot_on
        subtightplot(2,8,[11 12],gap,marg_h,marg_w)
    ...
    if plot_on
        subtightplot(2,8,[6 7],gap,marg_h,marg_w)
        plot_spk_raster(fig_props,RASTER)
        set(gca,'xticklabel',[])
        xlabel('')
        subtightplot(2,8,[14 15],gap,marg_h,marg_w)
   ...
   if plot_on
        subtightplot(2,8,[5 13],gap,marg_h,marg_w)
    ...
    if plot_on
        subtightplot(2,8,[8 16],gap,marg_h,marg_w)
        plot_tuning_curve_ephys(fig_props,tuning_curve)
