...
text(.55,.2,sprintf('Amp %.0f uV',-WAVEFORMS.amp),'units','normalized','Color','r','Background','w')
text(.55,.13,sprintf('tau %.0f us',1000*WAVEFORMS.tau_4),'units','normalized','Color','r','Background','w')
text(.55,.06,sprintf('SNR %.1f',WAVEFORMS.SNR),'units','normalized','Color','r','Background','w')

text(.55,.96,[num2str(WAVEFORMS.num_spikes) ' spks'],'Color','r','units','normalized','Background','w')
