...
for ij = 1:length(all_clust_ids)
   ...
   if plot_on
    ...
    keep_trials = keep_trials(ismember(keep_trials,find(session.trial_info.mean_speed >= 0 & ismember(groups_RASTER,group_ids_RASTER))));
    mean_ds = 2;
    temp_smooth = 80;
    ...
    % Make trial Raster to cor pos with laser on
    id_type_wall_tuning = 'olRLP';
    stim_name = 'corPos';
    keep_name = 'ol_base';
    [group_ids_RASTER groups_RASTER] = define_group_ids(exp_type,id_type_wall_tuning,trial_inds);
    keep_trials = trial_range;
    ...
    % Make tuning curve with laser on
    stim_name = 'corPos';
    keep_name = 'ol_base';
    id_type_wall_tuning = 'olRLP';
    tuning_curve = get_tuning_curve_ephys(clust_id,d,stim_name,keep_name,exp_type,id_type_wall_tuning,time_range);
  ...
  % Make trial Raster to laser power
    ...
    RASTER = get_spk_raster(spike_times_ephys,spike_trials,keep_trials,groups_RASTER,group_ids_RASTER,time_range,mean_ds,temp_smooth);
    if plot_on
        subtightplot(4,8,[19 20],gap,marg_h,marg_w)
        plot_spk_raster(fig_props,RASTER)
        set(gca,'xticklabel',[])
        xlabel('')
        subtightplot(4,8,[27 28],gap,marg_h,marg_w)
        plot_spk_psth(fig_props,RASTER);
        text(.02,.93,sprintf('%s',id_type_wall_tuning),'Units','Normalized','Color','r')
    end
    assignin('base','RASTER',RASTER);

    % Make laser power tuning curve
    stim_name = 'laser_power';
    keep_name = 'ol_base';
    id_type_wall_tuning = 'olLP';
    tuning_curve = get_tuning_curve_ephys(clust_id,d,stim_name,keep_name,exp_type,id_type_wall_tuning,time_range);
