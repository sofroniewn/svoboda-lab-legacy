function plot_spike_raster_groups(clustnum,sorted_spikes,groups,group_ids,col_mat)
...
for i_group = 1:num_groups-1
	...
	plot(spike_times_group,prev_trials+(trials_group)*100/max(trials),'.','Color',col_mat(i_group,:))
	prev_trials = prev_trials + max(trials_group)*100/max(trials) + 1;
end
xlim([0 5])
