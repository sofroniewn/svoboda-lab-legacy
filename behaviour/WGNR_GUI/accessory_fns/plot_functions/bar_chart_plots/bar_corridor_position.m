function bar_corridor_position(axes_handle,session,setup,frac_range,keep_trial_types,keep_inds,col_mat)
...
if setup == 1
    ...
    set(axes_handle,'ylim',[0 1])
else
    plot_handle = get(axes_handle,'Children');
    plot_handle = flipdim(plot_handle,1);
    %tmp = plot_handle;
    %plot_handle(1:2:end) = tmp(2:2:end);
    %plot_handle(2:2:end) = tmp(1:2:end);
end
...
for ij = keep_inds'
    vals = session.data{ij}.trial_matrix(3,:)./session.data{ij}.trial_matrix(4,:);
    ...
    if any(keep_range) == 1
        % mean_vals = mean(cor_pos(keep_range).*speed(keep_range))/mean(speed(keep_range));
