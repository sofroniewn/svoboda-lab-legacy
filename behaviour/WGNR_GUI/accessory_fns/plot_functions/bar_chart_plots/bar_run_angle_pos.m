function bar_run_angle_pos(axes_handle,session,setup,frac_range,keep_trial_types,keep_inds,col_mat)
...
for ij = keep_inds'
    ...
    if any(keep_range) == 1
        ...
        vals = atan2(session.data{ij}.processed_matrix(3,:) - session.data{ij}.processed_matrix(3,start_ind),session.data{ij}.processed_matrix(2,:) - session.data{ij}.processed_matrix(2,start_ind))*180/pi;
