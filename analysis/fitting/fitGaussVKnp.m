...
l_bound = [-inf      -10        0    0  -inf -ones(1,2) -ones(1,3)];
u_bound = [ inf   max(x(:))+10 100  inf   inf  ones(1,2) ones(1,3)];

%try
    estPrs = fmincon(@(prs) fitGaussVKnp_errFun(prs,x,y),initPrs,[],[],[],[],l_bound,u_bound,[],opts);
    out.estPrs = estPrs;
	out.predic = fitGaussVKnp_modelFun(x,estPrs);
