...
l_bound = [-inf      -10        0  0  -inf -ones(1,2)];
u_bound = [ inf   max(x(:))+10 100  inf   inf  ones(1,2)];

%try
    estPrs = fmincon(@(prs) fitGaussKnp_errFun(prs,x,y),initPrs,[],[],[],[],l_bound,u_bound,[],opts);
    out.estPrs = estPrs;
	out.predic = fitGaussKnp_modelFun(x,estPrs);
