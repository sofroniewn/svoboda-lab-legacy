function out = fitDoubleSigmoidKnp(x,y,initPrs)
...
l_bound = [-30  0  0    0       0     0       0  0 0];
u_bound = [ 30 100 1 max(x(:))  100 max(x(:)) 100  1  1];

%try
    estPrs = fmincon(@(prs) fitDoubleSigmoidKnp_errFun(prs,x,y),initPrs,[],[],[],[],l_bound,u_bound,[],opts);
    out.estPrs = estPrs;
	out.predic = fitDoubleSigmoidKnp_modelFun(x,estPrs);
