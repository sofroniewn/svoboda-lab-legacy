function out = fitGaussKGamma(x,y,initPrs)
...
l_bound = [ 0      0      0  -inf       0   0   0   0   0   0 ];
u_bound = [inf max(x(:)) 100  inf      inf inf inf inf  1  inf];

%try
    estPrs = fmincon(@(prs) fitGaussKGamma_errFun(prs,x,y),initPrs,[],[],[],[],l_bound,u_bound,[],opts);
    out.estPrs = estPrs;
	out.predic = fitGaussKGamma_modelFun(x,estPrs);
