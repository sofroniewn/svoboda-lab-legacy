function out = fitDoubleGaussKExp(x,y,initPrs)
...
l_bound = [     0      0  -inf      0       0   -inf     0   0   0   0 ];
u_bound = [max(x(:)) 100  inf  max(x(:))  100   inf     inf inf   1  inf];

%try
    estPrs = fmincon(@(prs) fitDoubleGaussKExp_errFun(prs,x,y),initPrs,[],[],[],[],l_bound,u_bound,[],opts);
    out.estPrs = estPrs;
	out.predic = fitDoubleGaussKExp_modelFun(x,estPrs);
